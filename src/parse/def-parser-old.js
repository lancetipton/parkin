...
const DEF_CLEAN_UP = [
  new RegExp(/\s*module.exports.*{([\s\S].*[\s\S]|[\s\S])+}/, 'gm'),
  new RegExp(/\s*import.*{[\s\S].*[\s\S]}.*from.*/, 'gm'),
  new RegExp(/\s*export.*{([\s\S].*[\s\S]|[\s\S]|)+}/, 'gm'),
]
/*
...
const cleanupContent = text => {
  ...
  return DEF_CLEAN_UP.reduce((cleaned, regex) => (cleaned.replace(regex, '')), content)
}
