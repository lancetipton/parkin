...
var esprima$1 = unwrapExports(esprima);
  ...
  require.define('/escodegen.js', function (module, exports, __dirname, __filename) {
    (function () {
      ...
      CodeGenerator.prototype.maybeBlockSuffix = function (stmt, result) {
        ...
        if (stmt.type === Syntax.BlockStatement && !stmt.leadingComments && !ends) {
          return [result, space];
        }
        if (ends) {
          return [result, base];
      ...
      CodeGenerator.Expression = {
        ...
        ConditionalExpression: function (expr, precedence, flags) {
          ...
          return parenthesize([this.generateExpression(expr.test, Precedence.Coalesce, flags), space + '?' + space, this.generateExpression(expr.consequent, Precedence.Assignment, flags), space + ':' + space, this.generateExpression(expr.alternate, Precedence.Assignment, flags)], Precedence.Conditional, precedence);
        },
...
const definitionParser = text => {
  const ast = esprima$1.parse(text);
  return ast.body.reduce((definitions, child) => {
    child.type === esprima$1.Syntax.ExpressionStatement && child.expression.type === esprima$1.Syntax.CallExpression && STEP_KEYS.includes(child.expression.callee.name) && definitions.push(escodegen.generate(child));
    return definitions;
  }, []);
};
