...
var __paramTypes = {
  ...
  word: {
    ...
    transformer: checkWorldValue((arg) => {
      return !isQuoted(arg) ? toStr(arg) : void 0;
    }, typeModel.type)
  },
  float: {
    ...typeModel,
...
var matchRegex = (definition, text) => {
  ...
  return match ? { definition, match: match.slice(1, match.length).filter(Boolean) } : emptyObj;
};
...
var getRegexParts = (defMatcher, opts = emptyObj) => {
  ...
  const parameters = includePartType("parameter" /* parameter */, opts, inArr, exArr) ? [...defMatcher.matchAll(new RegExp(RX_PARAMETER, "gi"))].map((match) => parseMatch(match, "parameter" /* parameter */, opts)) : emptyArr;
  const optionals = includePartType("optional" /* optional */, opts, inArr, exArr) ? [...defMatcher.matchAll(new RegExp(RX_OPTIONAL, "gi"))].map((match) => parseMatch(match, "optional" /* optional */, opts)) : emptyArr;
  const alts = includePartType("alternate" /* alternate */, opts, inArr, exArr) ? [...defMatcher.matchAll(new RegExp(RX_ALT, "gi"))].map((match) => parseMatch(match, "alternate" /* alternate */, opts)) : emptyArr;
...
var convertToRegex = (match, opts = emptyObj) => {
...
var attemptReplace = (match, world, location) => {
  ...
  return isFunc(replaceWith) ? replaceWith(world, location) : exists(replaceWith) ? replaceWith : match;
};
...
var registerFromCall = function(internalType, type, match, method, meta = noOpObj) {
...
//# sourceMappingURL=chunk-AH4VKI4X.mjs.map